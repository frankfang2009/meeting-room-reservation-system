...
class UpdateCheckUnitTests(unittest.TestCase):
    ...
    def test_parse_manifest_accepts_only_strict_schema(self) -> None:
        ...
        self.assertEqual(
            update_check.parse_manifest(_manifest("2.2.1"))["version"], "2.2.1"
        )
    ...
    def test_version_normalization_and_release_url(self) -> None:
        self.assertEqual(update_check.normalize_version("V2.2.1"), (2, 2, 1))
    ...
    def test_perform_check_reports_available_and_persists_state(self) -> None:
        ...
        self.assertEqual(summary["currentVersion"], "2.2.1")
    ...
    def test_perform_check_current_version_is_not_reported_as_update(self) -> None:
        server = _ManifestServer(_manifest("2.2.1"))
        self.addCleanup(server.close)
        ...
        self.assertEqual(summary["latestVersion"], "2.2.1")
    ...
    def test_view_disabled_and_corrupt_state(self) -> None:
        self.assertEqual(
            update_check.view(
                enabled=False, data_dir=self.data_dir, current_version="2.2.1"
            ),
            {"enabled": False},
        )
        ...
        summary = update_check.view(
            enabled=True, data_dir=self.data_dir, current_version="2.2.1"
        )
        self.assertEqual(summary["status"], "unknown")
