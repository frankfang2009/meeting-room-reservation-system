...
export function RoomAdminForm({ drawer, busy, deleteBusy, onSubmit, onFieldChange, onCancel, onDelete }) {
  const editing = drawer.type === "room-edit";
  return <form className="room-form" onSubmit={onSubmit} noValidate>
    <label className="field"><span>名称</span><input data-initial-focus disabled={busy} value={drawer.form.name} aria-invalid={Boolean(drawer.errors?.name)} aria-describedby={drawer.errors?.name ? "room-name-error" : undefined} onChange={(event) => onFieldChange("name", event.target.value)} />{drawer.errors?.name && <small id="room-name-error" role="alert">{drawer.errors.name}</small>}</label>
    <label className="field"><span>排序号</span><input type="number" min="1" max="10000" disabled={busy} value={drawer.form.sortOrder} aria-invalid={Boolean(drawer.errors?.sortOrder)} aria-describedby={drawer.errors?.sortOrder ? "room-sort-error" : undefined} onChange={(event) => onFieldChange("sortOrder", Number(event.target.value))} />{drawer.errors?.sortOrder ? <small id="room-sort-error" role="alert">{drawer.errors.sortOrder}</small> : <small className="field-hint">数字越小，在预约日历中越靠前</small>}</label>
    <label className="room-availability-toggle"><span><strong>在预约日历中启用</strong><small>停用后隐藏，已有预约不会自动取消。</small></span><input type="checkbox" disabled={busy} checked={drawer.form.isActive} onChange={(event) => onFieldChange("isActive", event.target.checked)} /></label>
    <label className="room-availability-toggle room-display-toggle"><span><strong>在公开大屏显示</strong><small>关闭后，该笔录室及其预约不会出现在公开引导中。</small></span><input type="checkbox" disabled={busy} checked={drawer.form.showOnDisplay} onChange={(event) => onFieldChange("showOnDisplay", event.target.checked)} /></label>
    <div className="room-form-actions"><button className="submit-button" type="submit" disabled={busy}>{busy ? "正在保存…" : editing ? "保存修改" : "创建笔录室"}</button><button className="secondary-button" type="button" disabled={busy} onClick={onCancel}>取消</button></div>
    {editing && <button className="room-delete-button" type="button" disabled={busy || deleteBusy} onClick={onDelete}><Trash size={17} />{deleteBusy ? "正在检查预约…" : "删除笔录室"}</button>}
  </form>;
}
...
export function UserAdminForm({ drawer, busy, lastAdminProtected, onSubmit, onFieldChange, onReset }) {
  ...
  return <form className="user-form" onSubmit={onSubmit} noValidate>
    ...
    <div className="user-form-footer"><button className="submit-button" type="submit" disabled={busy}>{busy ? "正在保存…" : editing ? "保存修改" : "创建用户"}</button></div>
