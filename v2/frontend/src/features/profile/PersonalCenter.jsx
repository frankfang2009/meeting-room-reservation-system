...
import { bookingDefaultsSummary } from "./settings-presentation.js";
...
function PreferencesView({ activeRooms, draft, durationSteps, errors = {}, onChange, onSave, onUiChange, tags, uiDraft }) {
  ...
  const [bookingDefaultsOpen, setBookingDefaultsOpen] = useState(false);
  ...
  const defaultSummary = bookingDefaultsSummary(draft, activeRooms, tags);
  ...
  return <form id="personal-settings-form" className="settings-layout" aria-label="个人设置" onSubmit={onSave}>
    ...
    <section className="settings-section settings-preferences-section"><h2>新建预约默认值</h2><button type="button" className="settings-disclosure-trigger" aria-label="编辑新建预约默认值" aria-expanded={bookingDefaultsOpen} onClick={() => setBookingDefaultsOpen((open) => !open)}><span>{defaultSummary}</span><small>仅新建时预填</small><CaretRight size={17} /></button>{bookingDefaultsOpen && <div className="settings-disclosure-content"><p>只会预填之后新建的空白预约，不影响编辑、恢复或冲突中的预约草稿。</p><div className="settings-choice-list"><label className="settings-choice-row"><span>默认预约时长</span><span className="settings-select-wrap"><select value={draft.defaultDuration || 60} onChange={(event) => onChange("defaultDuration", Number(event.target.value))}>{durationSteps.map((option) => <option value={option} key={option}>{option}分钟</option>)}</select><CaretRight size={18} /></span></label><label className="settings-choice-row"><span>默认笔录室</span><span className="settings-select-wrap"><select value={draft.defaultRoomId || ""} onChange={(event) => onChange("defaultRoomId", event.target.value)}><option value="">不指定</option>{activeRooms.map((room) => <option value={room.id} key={room.id}>{room.name}</option>)}</select><CaretRight size={18} /></span></label><label className="settings-choice-row"><span>默认标签</span><span className="settings-select-wrap"><select value={draft.defaultTagSlot ?? ""} onChange={(event) => onChange("defaultTagSlot", event.target.value ? Number(event.target.value) : null)}><option value="">不指定</option>{tags.map((tag) => <option value={tag.slot} key={tag.id}>{tag.label}</option>)}</select><CaretRight size={18} /></span></label></div></div>}</section>
    <section className="settings-section settings-personalization-section"><div className="settings-section-copy"><h2>使用习惯</h2><p>按需调整当前浏览器里的个人操作习惯。</p></div><button type="button" className="settings-disclosure-trigger" aria-expanded={usageOpen} onClick={() => setUsageOpen((open) => !open)}><span>更多使用习惯</span><small>当前：{uiDraft.defaultView === "calendar" ? "预约日历" : "我的预约"}</small><CaretRight size={17} /></button>{usageOpen && <div className="settings-disclosure-content"><p>以下选项只保存在当前浏览器，并按登录账号隔离。</p><div className="settings-choice-list"><label className="settings-choice-row"><span>登录后默认打开</span><span className="settings-select-wrap"><select value={uiDraft.defaultView} onChange={(event) => onUiChange("defaultView", event.target.value)}><option value="mine">我的预约</option><option value="calendar">预约日历</option></select><CaretRight size={18} /></span></label></div></div>}</section>
    ...
    <section className="settings-section settings-notifications-section"><h2>通知</h2><div className="settings-notification-list"><label className="settings-notification-row"><span><strong>预约变更</strong><small>页面打开时，修改或取消预约会通知我</small></span><input className="settings-switch" type="checkbox" checked={Boolean(draft.bookingChangeNotifications)} onChange={(event) => onChange("bookingChangeNotifications", event.target.checked)} /></label><div className="settings-notification-row"><label htmlFor="booking-reminder-switch"><strong>预约提醒</strong><small>页面打开时，开始前 {draft.reminderLeadMinutes || 30} 分钟提醒我</small></label><input id="booking-reminder-switch" className="settings-switch" type="checkbox" checked={Boolean(draft.bookingReminder)} onChange={(event) => onChange("bookingReminder", event.target.checked)} /></div>{draft.bookingReminder && <div className="settings-reminder-details"><label className="settings-notification-row"><span><strong>提醒提前量</strong><small>选择预约开始前的提醒时间</small></span><span className="settings-reminder-lead"><select aria-label="提醒提前量" value={draft.reminderLeadMinutes || 30} onChange={(event) => onChange("reminderLeadMinutes", Number(event.target.value))}>{[15, 30, 60].map((minutes) => <option value={minutes} key={minutes}>提前 {minutes} 分钟</option>)}</select><CaretRight size={16} /></span></label><label className="settings-notification-row"><span><strong>提醒提示音</strong><small>临近提醒与到达提醒到达时播放一声温和的轻提示</small></span><input id="reminder-sound-switch" className="settings-switch" type="checkbox" checked={draft.reminderSound !== false} onChange={(event) => onChange("reminderSound", event.target.checked)} /></label><p>工作交接仍会提示，不受这两项开关影响。</p></div>}</div></section>
    <section className="settings-section settings-reminder-template-section"><div className="settings-section-copy"><h2>对外提醒模板</h2><p>仅复制到剪贴板，由您自行发送。</p></div><button type="button" className="settings-disclosure-trigger" aria-expanded={reminderTemplateOpen} onClick={() => setReminderTemplateOpen((open) => !open)}><span>编辑模板</span><small>{reminderTemplate === DEFAULT_REMINDER_TEMPLATE ? "使用默认模板" : "已自定义"}</small><CaretRight size={17} /></button>{reminderTemplateOpen && <div className="settings-disclosure-content settings-template-content"><label className="settings-template-field"><span>模板内容 <small>{reminderTemplate.length}/200</small></span><textarea ref={reminderTemplateRef} maxLength={200} rows={4} value={reminderTemplate} aria-invalid={Boolean(errors.reminderTemplate)} aria-describedby={errors.reminderTemplate ? "reminder-template-error" : "reminder-template-help"} onChange={(event) => onChange("reminderTemplate", event.target.value)} />{errors.reminderTemplate && <small id="reminder-template-error" className="settings-field-error" role="alert">{errors.reminderTemplate}</small>}</label><div id="reminder-template-help" className="settings-template-variables" aria-label="可插入变量">{REMINDER_TEMPLATE_VARIABLES.map((variable) => <button type="button" onClick={() => insertTemplateVariable(variable.token)} key={variable.key}>{variable.token}</button>)}</div><div className="settings-template-preview"><span>示例预览</span><p>{renderReminderTemplate(reminderTemplate, REMINDER_TEMPLATE_PREVIEW_FIELDS)}</p></div><button className="settings-template-restore" type="button" onClick={() => onChange("reminderTemplate", DEFAULT_REMINDER_TEMPLATE)}>恢复默认</button></div>}</section>
  </form>;
}
