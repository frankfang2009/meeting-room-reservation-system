...
function PreferencesView({ activeRooms, draft, durationSteps, errors = {}, onChange, onSave, onUiChange, uiDraft }) {
...
export function PersonalCenter({
  ...
  return <main className="main-canvas settings-canvas personal-center-canvas" tabIndex={0}>
    <header className="page-header settings-header personal-center-header"><div><h1>个人中心</h1><p>查看工作概览与管理工作偏好</p></div>{tab === "preferences" && <div className="settings-header-actions"><button className="settings-logout-button" type="button" disabled={saving} onClick={onLogout}>退出登录</button><button className="settings-save-button" type="submit" form="personal-settings-form" disabled={saving}>{saving ? <><CircleNotch className="spin" size={17} />正在保存</> : "保存更改"}</button></div>}</header>
    ...
    {tab === "activity" ? <ActivityView activity={activity} onReload={onActivityReload} state={activityState} /> : <PreferencesView activeRooms={activeRooms} draft={draft} durationSteps={durationSteps} errors={errors} onChange={onChange} onSave={onSave} onUiChange={onUiChange} uiDraft={uiDraft} />}
  </main>;
}
