    ...
    try:
        ...
        if arguments.elevated_context:
            ...
            if os.name == "nt" and not is_admin() and not test_mode:
                # data/ 内的安装身份只对管理员开放；提权前读取它们会让标准用户
                # 或过滤令牌在 UAC 弹窗之前就失败。确认信息只来自对普通用户
                # 开放只读的版本文件，完整身份校验全部在提权后进行。
                _confirm(read_installed_version(install_root))
        ...
        return 6
    except (UpdatePolicyError, InstallerError, OSError, subprocess.TimeoutExpired) as error:
        print(f"\nV2 更新没有完成：{error}")
