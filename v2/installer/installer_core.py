...
class Bundle:
    ...
    def load(cls, tool_root: Path, *, _test_fixture: bool = False) -> "Bundle":
        ...
        if (
            ...
            raise InstallerError("安装包不是受支持的 V2.2.1 全新安装包")
    ...
    def _validate_service(value: Any) -> None:
        ...
        if not isinstance(value, dict) or value != expected:
            raise InstallerError("安装包服务管理契约不符合 V2.2.1 固定约定")
            ...
            try:
                ...
                with contextlib.suppress(OSError):
                    if empty_backup.exists():
                        empty_backup.rmdir()
                self.log.write("V2.2.1 全新安装完成；首次设置前服务保持回环访问")
                ...
                self.log.write(f"V2.2.1 安装失败：{error}", "ERROR")
