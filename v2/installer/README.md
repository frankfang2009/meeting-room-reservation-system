...
本目录实现 V2.2.2 全新安装、V2.1.0 起源离线累计升级与 macOS 自托管便携包，
...
python3 -m v2.installer.build_package \
  ...
  --output /absolute/path/to/会议室预约系统-V2.2.2-安装包.zip
```
...
只有零参数 `安装V2.2.2.bat`、安装说明和 `_V2安装工具`。SHA-256 只能证明与
...
python -m v2.installer.build_macos_dmg \
  --from-zip /absolute/path/to/output/会议室预约系统-V2.2.2-macOS-arm64.zip \
  --output /absolute/path/to/output/会议室预约系统-V2.2.2-macOS-arm64.dmg
```
