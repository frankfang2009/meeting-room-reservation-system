...
python3 -m v2.installer.build_package \
  ...
  --output /absolute/path/to/会议室预约系统-V2.5.1-安装包.zip
```
...
只有零参数 `安装V2.5.1.bat`、安装说明和 `_V2安装工具`。SHA-256 只能证明与
...
python -m v2.installer.build_macos_dmg \
  --from-zip /absolute/path/to/output/会议室预约系统-V2.5.1-macOS-arm64.zip \
  --output /absolute/path/to/output/会议室预约系统-V2.5.1-macOS-arm64.dmg
```
