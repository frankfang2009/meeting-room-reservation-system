...
"%MRV2_PS%" -NoProfile -NonInteractive -Command "try { $p=Start-Process -FilePath $env:MRV2_ELEVATE_SCRIPT -ArgumentList '--mrv2-uac-child' -Verb RunAs -Wait -PassThru; exit [int]$p.ExitCode } catch { exit 3 }"
set "START_EXIT=%errorlevel%"
if "%START_EXIT%"=="0" (
    start "" "http://127.0.0.1:8080/"
    echo.
    echo V2 系统已安全启动，浏览器已打开。
    echo.
    pause
)
if "%START_EXIT%"=="3" (
    echo.
    echo 已取消管理员授权，V2 服务没有被启动。
    pause
)
exit /b %START_EXIT%
...
:failed
echo.
echo V2 服务没有通过固定目录和 install_id 安全校验，未执行未知任务。
echo 请保留安装目录，联系维护人员。
echo.
pause
exit /b %START_EXIT%
