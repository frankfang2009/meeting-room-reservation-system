...
:failed
echo.
echo V2 开机任务没有通过固定目录和 install_id 安全校验。
echo 工具没有覆盖同名任务或防火墙规则。
echo.
pause
exit /b %SETUP_EXIT%
