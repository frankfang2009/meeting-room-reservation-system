...
chcp 65001 >nul
title 会议室预约系统 V1.0.3 候选更新

set "UPDATE_TOOL=%~dp0_V1.0.3更新工具"
set "PYTHONUTF8=1"
...
if "%UPDATE_RC%"=="0" (
    ...
    echo 另一个更新正在运行，请不要重复打开。
) else (
    echo 候选更新没有正常完成，返回代码：%UPDATE_RC%
    echo 请不要删除旧系统文件，把窗口提示和更新日志交给维护人员。
)
echo.
...
:missing
echo.
echo 更新工具不完整，请重新解压维护人员发送的完整 ZIP。
echo 不要单独复制这个 BAT。
echo.
if not "%MEETING_ROOM_UPDATE_NO_PAUSE%"=="1" pause
exit /b 1
