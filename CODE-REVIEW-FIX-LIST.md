...
- [ ] 位置：`v2/frontend/src/App.jsx:1588`（整块 loading 态替换画布）；骨架样式已存在未用
  （`calendar.css:212-263`）。
- [ ] 契约：DESIGN-CONTRACT:106 要求加载时保留日期控件、房间头、时间轴 + 安静骨架条。
- [ ] 期望：加载态保留框架+骨架条；切换日期时先清空/隔离旧 bookings，避免旧日期数据在新日期头部下闪现。
- [ ] 验收：production-source 断言 + 浏览器验证换日过程无旧数据闪现。
...
- [ ] 位置：筛选弹层（`App.jsx:1542,1577,1641`）、历史搜索、月份菜单、用户搜索等。
- [ ] 期望：与抽屉一致——Esc 关闭、点击外部关闭、关闭后焦点返回触发按钮。
  （可扩展 `useFocusTrap` 支持非模态 popover，或新增轻量 hook。）
- [ ] 验收：浏览器键盘/鼠标回归；如本轮引入 React Testing Library 可加交互测试。

### F15 触控目标 < 44px 违约 【P2 · 契约 127 行】

- [ ] 位置（实测）：抽屉返回钮 36×36（`drawer-shell.css:57-59`）；提醒"知道了" 34px
...
- [ ] 期望：全部 ≥44px（小图标可保留视觉尺寸、扩大命中区）。改完更新冻结 SHA。
- [ ] 验收：样式审计 + 1024×720 浏览器点按回归。
