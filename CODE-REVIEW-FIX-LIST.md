...
> **历史修复记录（已完成并冻结）。** 本清单保留 2026-08-14 至 2026-08-15 的
> 审查范围、修复依据和勾选结果，不是 V2.5 当前任务书。不要重新执行其中的旧分支、
> HEAD、E 编号或发布步骤，也不要把未勾选的后续构想当作 V2.5 当前缺陷。
> 当前产品真值见 `v2/docs/PRODUCT-CONTRACT.md`，当前发布门禁见
> `v2/docs/RELEASE-CHECKLIST.md`；Windows 验收仅从
> `v2/docs/V2.5.0-WINDOWS-PHYSICAL-ACCEPTANCE-GUIDE.md` 进入。
>
...
- [x] 抽屉打开时 toast 与提醒按钮移出交互范围（与 mainRef 一起 inert）；
- [x] 历史月份步进无下界 vs 下拉只列 12 个月——两者范围对齐；
- [x] 日历日期跳转限定业务范围（如今天 ±2 年）；
- [x] 历史筛选的房间/标签是单选却用复选框外观——改 radio 语义；
- [x] 个人中心"保存更改"加 busy/禁用，姓名必填校验；
- [x] 变更记录加载失败加"重试"入口（`App.jsx:846`）；
- [x] "已取消"徽章文字对比度 ≥4.5:1（`history.css:509-522`）；
- [x] 开始时间 30 分钟对齐的客户端预校验（`App.jsx:816`）；
- [x] 员工"前往预约日历"但无可用笔录室时给出提示而非静默落地（`App.jsx:1270-1271`）。
- [x] 验收：每项一个小断言或浏览器证据即可，别把这一组做成大重构。
...
### F19 契约漂移裁决（用户已于 2026-08-15 拍板）

- [x] 保留预约详情状态徽章；日期仍是唯一副标题文本，契约明确状态徽章在其相邻位置持续可见。
- [x] 保留 960/720/620px 响应式代码，定义为桌面窗口缩小时的防御性适配；不纳入正式验收，不承诺手机/平板支持。
- [x] 会话过期精确提示“本标签页内的预约草稿已保留，使用同一账号重新登录后可以恢复”。
