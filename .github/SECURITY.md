...
| 版本 | 状态 |
| --- | --- |
| `main` 上的 V2 源码 | 接受安全报告 |
| V2.1.0 自动构建候选 | 非正式发布，仅用于内部验证 |
| V1.x | 历史兼容维护，不接受新功能 |

当前没有完成签名和普通用户 Windows 10/11 验收的正式 V2 安装包。GitHub Actions 产物、分支 ZIP 或第三方重新打包文件都不应被视为项目正式发布。
...
V2.1.0 当前使用 HTTP，只支持可信的 Windows Domain/Private 局域网。直接暴露到互联网、访客网络或不受信任 Wi-Fi 不在支持范围。部署方仍负责操作系统补丁、主机账户、网络隔离、防火墙、终端防护、备份和物理安全。
